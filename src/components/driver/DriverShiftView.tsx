...
export default function DriverShiftView() {
  ...
  const allocation = allocations.find(
    (a) => a.driverId === activeDriverId && a.date === today,
  );
  const vehicle = allocation
    ? vehicles.find((v) => v.id === allocation.vehicleId)
    : null;
  const todayOrders = orders.filter(
    (o) => o.assignedDriverId === activeDriverId && o.deliveryDate === today,
  );
  ...
  function handleStartShift() {
    ...
    addToast({
      type: "success",
      title: "Shift Started",
      message: "Drive safe! Your deliveries are listed below.",
    });
  }
  ...
  function handleEndShift() {
    ...
    if (pendingDeliveries.length > 0) {
      if (
        !confirm(
          `You have ${pendingDeliveries.length} pending deliveries. End shift anyway?`,
        )
      )
        return;
    }
    ...
    addToast({
      type: "info",
      title: "Shift Ended",
      message: "Thanks for today's work!",
    });
  }
  ...
  if (!allocation) {
    return (
      <div className="card p-12 text-center animate-fade-in">
        ...
        <h2 className="text-lg font-display font-semibold text-surface-700 mb-2">
          No Allocation Today
        </h2>
        <p className="text-sm text-surface-500 max-w-sm mx-auto">
          {driver?.name} has no vehicle allocated for today. Contact admin to
          get a vehicle assigned.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 animate-fade-in">
          ...
          <div className="flex items-center justify-between">
            ...
            {allocation.startTime && (
              <span className="text-xs text-surface-500">
                ...
                {allocation.endTime &&
                  ` — Ended ${formatTime(allocation.endTime)}`}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="card p-3 text-center">
          <p className="text-xl font-display font-bold text-surface-900">
            {todayOrders.filter((o) => o.status === "failed").length}
          </p>
